...
public class IdConfig {
    ...
    public Id databaseId1(DataSource dataSource) {
        return DatabaseIdFactory.create().businessName(() -> String.format("test_db_%s", DateUtil.today())).prefix(() -> "2019-").dataSource(dataSource).step(1).stepStart(100000).retryTimes(3).tableName("id_test").getInstance();
    }

    @Bean
    public Id databaseId2(DataSource dataSource, IdDatabaseProperties databaseProperties) {
        return DatabaseIdFactory.create().businessName(() -> String.format("test_db_2_%s", DateUtil.today())).prefix(() -> "2019-2-").dataSource(dataSource).step(databaseProperties.getStep()).stepStart(databaseProperties.getStepStart()).retryTimes(databaseProperties.getRetryTimes()).tableName(databaseProperties.getTableName()).getInstance();
    }

    @Bean
    public Id redisId() {
        return RedisIdFactory.create().businessName(() -> String.format("test_redis_%s", DateUtil.today())).prefix(() -> "2019-").ip("localhost").port(6379).step(1).stepStart(0).getInstance();
    }
